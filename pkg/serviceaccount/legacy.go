...
func (v *legacyValidator) Validate(tokenData string, public *jwt.Claims, privateObj interface{}) (*ServiceAccountInfo, error) {
	...
	if v.lookup {
		...
		if serviceAccount.DeletionTimestamp != nil {
			...
			return nil, fmt.Errorf("service account %s/%s has been deleted", namespace, serviceAccountName)
		}
		if string(serviceAccount.UID) != serviceAccountUID {
			...
			return nil, fmt.Errorf("service account UID (%s) does not match claim (%s)", serviceAccount.UID, serviceAccountUID)
		}
	}
